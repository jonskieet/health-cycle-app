...
- **2026-07-27** — **Hoàn thành Module 12 — Xuất/Sao lưu dữ liệu (P12) + Module 13 — PWA cơ bản (F7), gộp 1 patch.** Đã triển khai:
  - **Module 12 (P12 — thay thế "sao lưu Dropbox")**:
    - `src/lib/export-data.ts` (mới, business logic thuần): `buildFullDataExport()` gộp profile + cycle_logs + health_metrics + appointments + reminders + kegel_sessions + fatigue_tests thành 1 object JSON, loại bỏ field nhạy cảm/nội bộ (`app_lock_pin_hash`, `is_vip`, `vip_activated_at`) trước khi xuất; `downloadFullDataExport()` tải file `.json` về máy qua Blob (client-side thuần, không qua server, giống cách `export-report.ts` làm PDF).
    - `src/lib/queries.ts`: thêm hàm thuần `fetchAllUserDataForExport(userId)` (không phải hook — chỉ chạy 1 lần khi bấm nút) lấy TOÀN BỘ lịch sử từng bảng (khác `useHealthMetrics()` vốn chỉ lấy 7 ngày gần nhất).
    - `src/app/settings/page.tsx`: thêm section "Dữ liệu" với nút "Xuất dữ liệu", không khoá VIP (dữ liệu vốn đã ở Supabase cloud sẵn, export chỉ là bản sao lưu cá nhân bổ sung, không phải tính năng cao cấp).
    - **Quyết định phạm vi quan trọng**: KHÔNG tích hợp Dropbox thật (cần OAuth + SDK bên thứ 3, ngoài phạm vi 1 module) và KHÔNG làm chức năng "nhập lại" (import) từ file JSON — chỉ làm chiều xuất, vì nhập lại cần xử lý trùng lặp/validate phức tạp và rủi ro ghi đè dữ liệu thật.
  - **Module 13 (F7 — PWA cơ bản, thay thế "widget màn hình chính")**:
    - `public/manifest.json` (mới): Web App Manifest chuẩn (tên, theme color theo `--c-period`, icon).
    - `public/icon-192.png`, `public/icon-512.png` (mới): icon tối giản tạo tự động, nên thay bằng icon thiết kế thật trước khi phát hành.
    - `public/sw.js` (mới): service worker tối giản — chỉ cache app-shell tĩnh để mở được app khi mất mạng; cố ý KHÔNG cache API/Supabase để tránh hiển thị dữ liệu sức khỏe cũ tưởng là mới.
    - `src/components/layout/PwaRegister.tsx` (mới): đăng ký service worker + dùng App Badging API (`navigator.setAppBadge`) hiển thị số ngày còn lại đến kỳ kinh tiếp theo trên icon app khi đã "Add to Home Screen" — thay thế gần nhất cho "widget màn hình chính" trên nền web; API không hỗ trợ thì tự bỏ qua.
    - `src/app/layout.tsx`: khai báo `manifest`/`appleWebApp` trong `metadata`, thêm `viewport.themeColor`, render `<PwaRegister />`.
  - Đã cài `npm install`, chạy `tsc --noEmit` + `eslint` trên toàn bộ file mới/sửa — không lỗi. `next build` chưa chạy được trong sandbox do mạng chặn Google Fonts — không liên quan tới code patch.
  - **Còn thiếu để hoàn thiện đầy đủ**: (Module 12) chưa có import lại dữ liệu từ file backup; (Module 13) icon PWA là hình tối giản tự tạo cần thay bằng thiết kế thật; chưa có UI chủ động gợi ý "Add to Home Screen"; service worker chưa có chiến lược cache nâng cao (stale-while-revalidate).
  - Người thực hiện: Claude. File package gửi cho user: `module12_module13_export_pwa.zip` (2 file mới `src/lib`, 1 file mới `src/components/layout`, 4 file mới `public`, 3 file sửa: `queries.ts`, `settings/page.tsx`, `layout.tsx`).
- **2026-07-27** — **Quyết định của chủ dự án: bỏ qua 2 mục còn lại trong roadmap — P11 (Đa hồ sơ) và F9 (Đăng nhập ẩn danh).** Không code. Lý do: P11 không cần thiết với quy mô app cá nhân hiện tại; F9 mâu thuẫn với kiến trúc auth đã xây (AuthGate, App Lock PIN, VIP, reminders đều gắn theo `user_id` thật), chủ dự án chọn không đổi kiến trúc để làm. **Roadmap coi như đã hoàn tất phạm vi triển khai** — không còn module nào đang chờ ở mục 3/4. Nếu muốn mở lại P11/F9 sau này, cần agent mới đọc kỹ lại quyết định này trước khi bắt đầu (không phải "bị bỏ sót", mà là lựa chọn có chủ đích).
  - Người thực hiện: Claude (theo xác nhận trực tiếp từ chủ dự án).
